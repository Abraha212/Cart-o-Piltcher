...
export default function Footer() {
  ...
  return (
    <footer ref={footerRef} className="bg-[#0F172A] py-12">
      <div className="max-w-[1200px] mx-auto px-5 md:px-20">
        ...
        <div className={`text-center mb-6 transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
          <img
            ...
            className="h-11 w-auto mx-auto transition-transform duration-300 hover:scale-105"
          />
        </div>
        ...
        <nav className="flex flex-wrap justify-center gap-9 mb-6">
          {links.map((link, i) => (
            <a
              key={link.label}
              href={link.href}
              className={`text-white text-[15px] hover:text-[#CDFF00] transition-all duration-300 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}
        ...
        <div className={`text-center transition-all duration-700 ${isVisible ? 'opacity-100' : 'opacity-0'}`} style={{ transitionDelay: '600ms' }}>
          ...
          <p className="text-white/50 text-sm mt-2 hover:text-[#CDFF00] transition-colors duration-300">
